            ...
            <form id="update_form">
                ...
                <div class="row">  
                    <div class="col-sm-3">
                        <div class="form-group">
                            <label class="switch">
                                <input type="checkbox" class="group_name group-{{ $group->group_name}}" {{ App\Models\Functions::rolesHasPermissions($role, $permissions) ? 'checked' : '' }}>
                                <span class="slider round"> </span>
                            </label>
                ...
                @foreach ($permissions as $permission)
                            ...
                            <div class="col-sm-2">
                                <div class="form-group">
                                    <label class="switch">
                                        <input type="checkbox" class="group-{{ $group->group_name}}" id="permission_box{{ $permission->id}}" name="permission_id[]" {{ $role->hasPermissionTo($permission->name) ? 'checked' : '' }} value="{{ $permission->id}}">
                                        <span class="slider round"> </span>
                                    </label>
                                    <label class="control-label" >{{ ucwords(str_replace('_', ' ', $permission->name))}}</label>
                                </div>
                            </div>
                @endforeach
                        </div>
                ...
                @endforeach
                <button class="btn btn-primary" id="update_btn">Update Role Permissions</button>
            </form>
            <!--===================================================-->
        </div>
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        if (response.status == 200) {
                            toastr.success('Patient Data Updated Successfully!', 'Successfull!');
                            $('#update_btn').prop('disabled', false);
                            $('#update_btn').text('Update Role Permissions');
