...
@section('page_head')
    <div class="row">
        <div class="col-md-6 table-toolbar-left" style="padding-bottom: 5px;">
            ...
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <div id="page-title">
                <h1 class="page-header text-overflow">Permission Groups</h1>
            </div>
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <!--End page title-->
            ...
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <ol class="breadcrumb">
            <li>Manage All Permission Groups ( Add - Update - Delete )</li>
            </ol>
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <!--End breadcrumb-->
        </div>
            ...
            <div class="form-inline">
                <div class="row">
                    ...
                    <div class="col-sm-6 table-toolbar-right" style="padding-bottom: 2px;">
                        <button id="lounch_add_modal_btn" data-target="#add_modal" data-toggle="modal" class="btn btn-purple"><i class="demo-pli-add"></i> Add Permission Group</button>
        ...
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" id="add_btn">Save Permission Group</button>
    ...
    <div class="modal fade" id="update_modal" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><i class="pci-cross pci-circle"></i></button>
                    <h4 class="modal-title">Update Permission Group</h4>
                </div>

                <!--Modal body-->
                <div class="modal-body pb-0" style="min-height: auto">
                    <!--===================================================-->
                    <form id="update_permission_group_form">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <input type="hidden" id="permission_group_id" name="permission_group_id" class="form-control textbox" placeholder=" ">
                                    <input type="text" id="permission_group_name" name="permission_group_name" class="form-control textbox" placeholder=" ">
                                    <label class="control-label textboxlabel" >Permission Group Name</label>
                                </div>
                            </div>
                        </div>
                ...
                <!--Modal footer-->
                <div class="modal-footer">
                    <button class="btn btn-primary" id="update_btn">Update Permission Group</button>
                    <button data-dismiss="modal" class="btn btn-default" type="button">Close</button>
                </div>
            </div>
        </div>
    ...
    $(document).ready(function(){
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        if (response.status == 200) {
                            ...
                            $('#add_btn').text('Save Permission Group');
                            $('#add_btn').prop('disabled', false);
                            $('#add_modal').modal('hide');
                            loadData();
                        }else if (response.status == 'groupExist') {
                            Swal.fire({ 
                                    title: "Permission Group Exist!",
                                    text: "This Permission Group Already Exist!",
                                    icon: "error", 
                                    buttonsStyling: !1,
                                    confirmButtonText: "Ok, got it!",
                                    customClass: { 
                            ...
                            Swal.fire({ 
                                    title: "Error!",
                                    text: "Save Permission Group Error Try Again!",
                                    icon: "error", 
                                    buttonsStyling: !1,
                                    confirmButtonText: "Ok, got it!",
                                    customClass: { 
        ...
        $(document).on("click", '#edit_btn', function(event){
            ...
            $.ajax({
                ...
                {
                    console.log(data);
                    $('#permission_group_id').val(data.data.id);
                    $('#permission_group_name').val(data.data.permission_group_name);
        ...
        $("#update_permission_group_form").validate({
            rules: {
                permission_group_name: {
                    required: true,
                },
        ...
        $("#update_btn").click(function(e){
            e.preventDefault();
            if($("#update_permission_group_form").valid()){
                $(this).prop('disabled', false);
                $(this).text('Updating..');
                //Get Data From Modal Form On Click Save Button
                var formData = new FormData($('#update_permission_group_form')[0]);

                $.ajax({
                    url: '{{ route("update_permission_group") }}',
                    method: 'post',
                    data : formData,
                    contentType : false,
                    processData : false,
                    ...
                    success:function(response){
                        console.log(response);
                        if (response.status == 200) {
                            toastr.success('Permission Group Updated Successfully!', 'Successfull!');
                            $('#update_modal').find('input').val('');
                            $('#update_btn').prop('disabled', false);
                            $('#update_btn').text('Update Permission Group');
                            ...
                            Swal.fire({ 
                                    title: "Error!",
                                    text: "Update Permission Group Error Try Again!",
                                    icon: "error", 
                                    buttonsStyling: !1,
                                    confirmButtonText: "Ok, got it!",
                                    customClass: { 
        ...
        $(document).on("click", '#delete_btn', function(event){
            ...
            Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        var element = this;
                        $.ajax({
                            url: 'delete_permission_group/'+permission_id,
                            type : "GET",
                        success : function(response){
                            if (response.status == 200)
                            {
        ...
        var defaultOptions = {
            ajax: {
                url: '{{ route("load_permission_groups") }}',
                type: 'post',
                data: { csrf: csrf, startdate: "", enddate: "", search: "", type: "" }
            },
            columns: [
                { data: "id" },
                { data: "permission_group_name" },
                { data: "btn" },
